...
if (isset($_POST['login'])) {
  ...
  /*if (isset($_POST['login'])) {
    ...
    if ($check_customer == 1) {
        $row = $result->fetch_assoc();
        $cliente_id = $row['cliente_id']; // Obtener el cliente_id del resultado de la consulta del cliente
        $stored_pass = $row['cliente_pass'];
        ...
        if($row['activo'] == 0){
            ...
            if (password_verify($cliente_pass, $stored_pass)) {
                $_SESSION['cliente_email'] = $cliente_email;
                echo "<script>alert('El inicio de sesión ha sido exitoso')</script>";
                ...
                $select_cart = "SELECT * FROM cart WHERE cliente_id = ?";
                $stmt_cart = $con->prepare($select_cart);
                $stmt_cart->bind_param("i", $cliente_id); // Cambiar el tipo de parámetro a entero
                $stmt_cart->execute();
                $run_cart = $stmt_cart->get_result();
                $check_cart = $run_cart->num_rows;
